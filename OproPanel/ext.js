...
function updateThemeWithAppSkinInfo(appSkinInfo) {
    ...
    var isPanelThemeLight = panelBackgroundColor.red > 128; // Updated threshold for better accuracy

    var fluentProvider = document.querySelector('fluent-provider');
    if (fluentProvider) {
        if (isPanelThemeLight) {
            fluentProvider.setAttribute('theme', 'light');
        } else {
            fluentProvider.setAttribute('theme', 'dark');
        }
    }
}
